...
public:
	...
	map<node, int> query_topk_label();
	map<uint32_t, int> query_neighbors_label(node keyN);
};
...
void LSketch::update(string s1, string s2, int weight, string l_A, string l_B, string l_e)
{
	...
	{
		set<node> nei;
		nei.insert(node(stoi(s1), l_A));
		neighbors[key] = nei;
	}
	else
	{
		set<node> nei = neighbors[key];
		nei.insert(node(stoi(s1), l_A));
		neighbors[key] = nei;
	}
	...
	if (flag == 0)
	{
		if (heap[0].value < in_weight)
		{
			...
			Update_heap_label(); // update HEAP, and flag in CM
...
void LSketch::Update_heap_label()
{
	uint32_t tmp_value;
	uint32_t tmp_index;
	for (uint32_t i = 0; i < k; i++)
	{
		if (heap[i].label != "")
		{
			...
			int weight = sketch->nodeValueQuery(to_string(heap[i].key), 1, heap[i].label);
...
map<node, int> LSketch::query_topk_label()
{
	map<node, int> result;
	// cout << "result:" << endl;
	// update
	...
	for (uint32_t i = 0; i < k; i++)
	{
		// cout << "key: " << key << "\t";
		// cout << "value: " << heap[0].value << endl;
		node tmp(heap[0].key, heap[0].label);
		result.insert(make_pair(tmp, heap[0].value));
		heap[0].key = 0;
		heap[0].value = 1 << 29;
		MINHEAPIFY(0);
	}
...
map<uint32_t, int> LSketch::query_neighbors_label(node keyN)
{
	map<uint32_t, int> result;
	// cout << "result:";
	// 构建neighbor的heap
	...
	uint32_t ending = keyN.key;
	// cout<<neighbors.count(ending)<<endl;
	// set<node> tmp = neighbors.at(ending);
	set<node> tmp = neighbors[ending];
	
	for (set<node>::iterator it = tmp.begin(); it != tmp.end(); ++it)
	{
		int neighbor_weight = sketch->edgeQuery(to_string((*it).key), (*it).label,to_string((keyN).key), (keyN).label);
